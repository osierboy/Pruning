...
class TrainPrueFineTuner:
    def __init__(self, train_data, test_data, model):
        ...
        self.pruner = FilterPrunner(self.model)
    ...
    def prune(self):
        ...
        for _ in range(iterations):
            ...
            # print(self.taylorData.train_acc1)
            # print(self.taylorData.train_acc2)
            # print(self.taylorData.test_acc1)
            # print(self.taylorData.test_acc2)
            ...
            if _ + 1 == rate_iter[0]:
                ...
                for layer_index in self.taylorData.layer_prune_0:
                    self.taylorData.layer_prune_98[layer_index] = self.taylorData.layer_prune_0[layer_index] - \
                                                                  self.taylorData.layer_prune[
                                                                      layer_index]  # 剪枝98%后每一层剩余的filters个数
...
class FilterPrunner:
    ...
    def compute_rank(self, grad):   # 被forward调用，用于grad更新时，计算值用于排序
        ...
        values = torch.sum((activation * grad), dim = 0).sum(dim = 1).sum(dim = 1).data      # 求和，结果为维度为1的向量
        values = values / (activation.size(0) * activation.size(2) * activation.size(3))     # 求取均值
